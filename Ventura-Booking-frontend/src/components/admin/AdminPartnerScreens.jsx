...
const AdminPartnerScreens = ({ partnerId }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h2 className="text-xl font-bold">Theater Screens & Schedules (Admin Mode)</h2>
                    <p className="text-sm text-gray-500">Manage screens on behalf of partner.</p>
                </div>
                <button onClick={() => {
                    setScreenData({ name: '', screenType: '2D', seatCapacity: '', rows: '', columns: '' });
                    setEditingScreenId(null);
                                                ...
                                                <div key={st._id} className="flex justify-between items-center hover:bg-gray-50 p-1.5 rounded text-sm group">
                                                    <div className="flex items-center gap-2">
                                                        <span className="font-mono font-bold bg-gray-100 px-2 rounded text-gray-800">
                                                            {st.date ? new Date(st.date).toLocaleDateString() : ''} {formatTime12Hour(st.time)}
                                                        </span>
                        ...
                        <div className="flex gap-4 items-start">
                            <div className="w-48 flex flex-col gap-1" style={{ paddingTop: 'calc(2rem + 1px)' }}>
                                ...
                                {Array.from({ length: managingScreen.rows }).map((_, r) => {
                                    ...
                                    return (
                                        <div key={rowLabel} className="h-8 flex items-center gap-1">
                                            <input
                                                type="number"
...
export default AdminPartnerScreens;
