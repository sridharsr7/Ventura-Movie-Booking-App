...
const PartnerReports = () => {
    ...
    const [activeTab, setActiveTab] = useState(() => localStorage.getItem('partnerReports_activeTab') || 'pending');
    const [dateFilter, setDateFilter] = useState(() => localStorage.getItem('partnerReports_dateFilter') || 'all');
    const [customDate, setCustomDate] = useState(() => JSON.parse(localStorage.getItem('partnerReports_customDate')) || { start: '', end: '' });
    const [cityFilter, setCityFilter] = useState(() => localStorage.getItem('partnerReports_cityFilter') || 'all');
    const [searchQuery, setSearchQuery] = useState(() => localStorage.getItem('partnerReports_searchQuery') || '');

    useEffect(() => {
        localStorage.setItem('partnerReports_activeTab', activeTab);
        localStorage.setItem('partnerReports_dateFilter', dateFilter);
        localStorage.setItem('partnerReports_customDate', JSON.stringify(customDate));
        localStorage.setItem('partnerReports_cityFilter', cityFilter);
        localStorage.setItem('partnerReports_searchQuery', searchQuery);
    }, [activeTab, dateFilter, customDate, cityFilter, searchQuery]);
    ...
    const fetchReports = async () => {
        try {
            ...
            console.error("Error fetching partner reports:", err);
            setLoading(false);
        }
    };
    ...
    const uniqueCities = [...new Set(reports.map(r => r.partner?.location).filter(Boolean))];

    const availablePartners = [...new Set(reports
        .filter(r => cityFilter === 'all' || r.partner?.location === cityFilter)
    ...
    const getFilteredReports = () => {
        ...
        if (cityFilter !== 'all') {
            filtered = filtered.filter(r => r.partner?.location === cityFilter);
        }
    ...
    return (
        <div className="animate-fade-in p-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Partner Reports</h2>

            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6 space-y-4 md:space-y-0 md:flex md:flex-wrap md:items-center md:justify-between gap-4">

                <div className="flex bg-gray-100 p-1 rounded-lg w-max overflow-x-auto">
                    {['pending', 'viewed', 'resolved'].map(tab => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`px-6 py-2 rounded-md text-sm font-semibold transition-all capitalization whitespace-nowrap ${activeTab === tab
                                ? 'bg-white text-gray-800 shadow-sm'
                                : 'text-gray-500 hover:text-gray-700'
                                }`}
                        >
                            {tab === 'pending' ? 'new' : tab === 'resolved' ? 'completed' : tab} ({reports.filter(r => r.status === tab).length})
                        </button>
                    ))}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-10 pr-3 py-2.5 bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 outline-none w-full md:w-64"
                        />
                    ...
                    <select
                        ...
                        className="bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block p-2.5 outline-none min-w-[150px]"
                    >
                ...
                <div className="grid gap-6">
                    {filteredReports.length > 0 ? (
                        filteredReports.map((report) => (
                            <div
                                key={report._id}
                                className={`bg-white rounded-xl shadow-sm border p-6 transition-all border-l-4 ${report.status === 'pending' ? 'border-l-red-500 hover:shadow-md' :
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        ...
                                        <div className="text-sm text-teal-700 font-semibold mb-1">
                                            Partner: {report.partner?.name || 'Unknown'}
                                            <span className="text-gray-400 font-normal ml-2">({report.partner?.email || 'No Email'})</span>
                                        </div>
                                        <div className="text-xs text-gray-500 mb-1 flex items-center gap-1">
                                            <span className="material-symbols-outlined text-[14px]">location_on</span>
                                            {report.partner?.location || 'Unknown Location'}
                                        </div>
                                ...
                                <div className="flex justify-end gap-3 pt-2 border-t border-gray-50">
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/admin/dashboard/partner-reports/${report._id}`);
                                        }}
                        ...
                        <div className="text-center py-20 bg-white rounded-xl border border-gray-100 border-dashed">
                            ...
                            <p className="text-gray-500">No {activeTab} reports found {dateFilter !== 'all' ? `for ${dateFilter}` : ''} {cityFilter !== 'all' ? `in ${cityFilter}` : ''} {searchQuery ? `matches "${searchQuery}"` : ''}.</p>
                        </div>
                    )}
                </div>
            )}
